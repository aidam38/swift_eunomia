...
#  f_slow = 1./2500.;	# yr^-1; I generally do NOT want frequencies larger than this limit
#			# (ALL secular frequencies are definitely lower!)

  f_slow = 1./20001.;	# yr^-1; I generally do NOT want frequencies larger than this limit
...
(FNR==2){
  if (length($1) != nfilters) {
    print "ERROR: NUMBER OF FILTERS IS NOT " nfilters "!!!";
    exit;
  }
...
(FNR==3){
  gsub("!.*", "");
  if (NF != nfilters) {
    print "ERROR: NUMBER OF DECIMATION FACTORS IS NOT " nfilters "!!!";
    exit;
  }
  for (i=1; i<=nfilters; i++) {
    K[i] = $i;
