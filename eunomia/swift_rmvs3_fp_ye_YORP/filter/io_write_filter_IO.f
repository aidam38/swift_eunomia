...
c  a slow variable sigma = (p+q)/q*lambda_PL - p/q*lambda_TP - varpi_TP
     ...
     :    + osc_elmts(5,filter_plid) + osc_elmts(6,filter_plid))
        sigma = zero2pi(dble(filter_p+filter_q)/filter_q * lambda_PL
     :    - dble(filter_p)/filter_q * lambda
     :    - varpi)
        elmts(FN(1),6,id,1) = cos(sigma)
        elmts(FN(1),7,id,1) = sin(sigma)
...
c          elmts(FN(1),6,id,1) = e
c          elmts(FN(1),7,id,1) = inc
...
c          call io_open(iu,oname,'append','UNFORMATTED',ierr)
     ...
     &      filter_A,filter_MA,filter_write,filter_write_r8)
        else    ! if (filter_stat(n:n).eq.'B') then
          call filter_elmts_write(iu,elmts,n,nbod,ntp,istat,
     &      filter_B,filter_MB,filter_write,filter_write_r8)
        endif
