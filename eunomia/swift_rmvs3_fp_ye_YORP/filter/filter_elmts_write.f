...
c
      ...
      logical filter_write,filter_write_r8
c temp vars
      integer i,id
...
c  I did'nt parallelize these 2 cycles, because of i/o routines.
      do i=2,nbod
        ...
        s_c = filter_filter(d,elmts(1,6,id,n),FM)
        s_s = filter_filter(d,elmts(1,7,id,n),FM)
...
c          e = filter_filter(d,elmts(1,6,id,n),FM)
c          inc = filter_filter(d,elmts(1,7,id,n),FM)
          e = sqrt(k*k+h*h)
          inc = 2.0d0 * asin(sqrt(q*q+p*p))
