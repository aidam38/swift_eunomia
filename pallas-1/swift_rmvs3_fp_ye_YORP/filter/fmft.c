...
/* Slightly modified to be callable from FORTRAN77 subroutines. */
/* by Miroslav Broz (miroslav.broz@email.cz), Apr 1st 2003 */

/* These two dimensions have to be EXACTLY the same as in proper.inc!!! */
#define MAX_FMFT_NFREQ 100
#define MAX_FMFT_NDATA 512
...
/* Parameters as pointers due to Fotran77 calling. MB, Mar 18th 2003 */

int fmft_(double output[][MAX_FMFT_NFREQ], int *p_nfreq, double *p_minfreq, double *p_maxfreq,
  int *p_flag, double input[][MAX_FMFT_NDATA], long *p_ndata)
...
/* by Miroslav Broz (miroslav.broz@email.cz), Nov 12th 2002 */
  int nfreq, flag;
  long ndata;
...
/* do NOT stop ever! Miroslav Broz (miroslav.broz@email.cz), Mar 30th 2005 */
  if(maxpow == 0) {
    printf("DFT has no maximum ...\n");
    maxj = 1;
    maxpow = powsd[1];
  }
