...
c****************************************************************
      subroutine fmft_call(input, minfreq, maxfreq, data_sep,
     :  min_gs, max_gs, freq, amp)
c****************************************************************
c
...
c
      ...
      real*8 freq, amp                  ! proper frequency and amplitude
...
c optionally, save ALL frequencies, amplitudes and phases, like in propgs output
     ...
     :      *((180.d0/pi)*3600.d0)/data_sep
          ...
          phase = output(j+1, 3*fmft_flag+1) *180.d0/pi
          if (phase.lt.0) phase = phase + 360.d0
          gsoutput(j, 3) = phase
        enddo
      endif
...
c  discard well known planetary frequences
          ...
          do k = 1, fmft_nfreqknown
            if (abs(freq - fmft_freqknown(k)).lt.fmft_freqtoler) then
              fmft_print = .false.
            endif
          enddo
        endif
...
c          phase = output(i, 3*fmft_flag+1) *180.d0/pi
c          if (phase.lt.0) phase = phase + 360.d0
...
c        phase = atan2(vectadd_x, vectadd_y) *180.d0/pi
      endif
