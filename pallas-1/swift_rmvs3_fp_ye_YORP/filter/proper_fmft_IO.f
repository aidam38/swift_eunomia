      ...
      subroutine proper_fmft(t,tstart,tstop,nbod,ntp,istat,oname,iu)
c
...
c  if it is so, drop the current data and wait for the next buffer
          if (fmft_restart) then
            call proper_shift(tstart,tstop,PN,nbod2,ntp,
...
c  p and q first
          ...
          call fmft_call(input,minfreq,maxfreq,data_sep,
     :      minfreq,maxfreq,freq,amp)
          e = amp
          g(id) = freq
          call fmft_gsout(goutput, id)
...
c  p and q first
            ...
            call fmft_call(input,minfreq,maxfreq,data_sep,
     :        fmft_ming,fmft_maxg,freq,amp)
            e = amp
            g(i) = freq
            call fmft_gsout(goutput, i)
...
c  k and h second
            ...
            call fmft_call(input,minfreq,maxfreq,data_sep,
     :        fmft_mins,fmft_maxs,freq,amp)
            inc = 2.0d0*asin(amp)
            s(i) = freq
            call fmft_gsout(soutput, i)
...
c  BEWARE OF UNIT NUMBERS! 
c  optionally write frequencies
        if (fmft_writegs) then
          call io_write_fmft(tout,nbod2,ntp,istat,g,s,iu+1,'append')
        endif

c  optionally write frequencies, amplitudes and phases
        if (fmft_propgs) then
          call io_write_fmft_propgs(tout,nbod2,ntp,istat,
     :      goutput,soutput,iu+2,'append')
        endif
